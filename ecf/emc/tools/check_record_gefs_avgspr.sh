...
fdir=$COM/$run.$YMD/$CYC/pgrb2ap5_bc

echo " prod 28224(per 4704) records"
echo " "
...
do
    ...
    line=`$WGRIB2 $file | grep "hour fcst" | wc -l`
    tline=`expr $tline + $line`
    echo "$px$prod.t${CYC}z.pgrb2a.0p50_bcf$hr has records of $line"
    done
    echo " "
