...
#rm namin_avgspr_${nfhrs}
  ...
  do
    ...
    if [ $nfhrs -le 99 ];then
      file=$COMIN/ENSEMBLE.MET.fcst_et0${fensmem}.0${nfhrs}.${PDY}${cyc}
      if [ $nfhrs -eq 00 ];then
        file=$COMIN/ENSEMBLE.MET.fcst_et0${fensmem}.000.${PDY}${cyc}
      fi
    else
      file=$COMIN/ENSEMBLE.MET.fcst_et0${fensmem}.${nfhrs}.${PDY}${cyc}
    fi
    if [ -s $file ]; then
      (( ifile = ifile + 1 ))
      iskip=0
  ...
  echo " cfopg1='fnmoc_geavg.t${cyc}z.pgrb2af${nfhrs}'," >>namin_avgspr_${nfhrs}
  echo " cfopg2='fnmoc_gespr.t${cyc}z.pgrb2af${nfhrs}'," >>namin_avgspr_${nfhrs}
...
if [ "$SENDCOM" = "YES" ]; then
  for nfhrs in $hourlist
  do
    file=fnmoc_geavg.t${cyc}z.pgrb2af$nfhrs
    if [ -s $file ]; then
      mv $file $COMOUT/
    fi
    file=fnmoc_gespr.t${cyc}z.pgrb2af$nfhrs
    if [ -s $file ]; then
      mv $file $COMOUT/
    fi
  done
