...
! Part I: get ctl + 20 ensemble members precipitation data
  ...
  do m=1, mem  
    ...
    ! read in control member for m=1, ipdt(16)=1 and ipdt(17)=0                  
    ! ipdt(16)=4: Multi-model forecast

    if(m.eq.1) then
      ipdt(16)=1           ! type of ensemble forecast          
      ipdt(17)=0           ! perturbation number               
    else
      ipdt(16)=4           ! type of ensemble forecast          
      ipdt(17)=m-1         ! perturbation number                 
    endif

    ! ipdt(8) indicator of unit of time range 
    ! CMC ipdt(8)=1: hour

    ipdt(8)=1       

    ! ipdt(9): forecast time in units
    ! for CMC prcp, ipdt(9) is the start period

    ipdt(9)=int((n-1)*6) 

    ! ipdt(29): indicator of unit of time range for process
    ! 11: 6 hours; 1: hour; 10: 3 hour; 12: 12 hours
    ! CMC has ipdt(29)=1

    ipdt(29)=1         

    ! ipdt(30): length of the time range over which post-processing is done
    ! for this code, choose 6 hrly forecast

    ipdt(30)=6           ! length of the time range is 6 hours

    ipdtn=11; igdtn=-1; iret=99
    ...
    if(iret.eq.0.and.icnt.eq.1) then
...
!
  if(icnt.le.1) then
    call gf_free(gfld)
    print *, '----- Total member 0 or 1, stop for next step ------'
  endif
