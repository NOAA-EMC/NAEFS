...
echo "         Aug. 2015 - Modified for 6km and 3.0km ALASKA"
echo "------------------------------------------------------------------------------------"
###########################################################################################
...
if [ "$SENDCOM" = "YES" ]; then
  ...
  #
  #  move downscaled alaska products at 3km including grib2
  #
...
if [ "$SENDDBN" = "YES" ]; then
      ...
      if [ "$runlabel" = "late" ]; then
        for nfhrs in $hourlist; do
          for nens in $prodlist; do
            ...
            ifile=naefs.t${cyc}z.${nens}.f${nfhrs}.alaska_3p0.grib2    
  ...
  if [ "$IFGEFS" = "YES" ]; then
    for nfhrs in $hourlist; do
      for nens in $prodlist; do
        if [ -s poe_alert.${nfhrs}.${nens} ]; then rm poe_alert.${nfhrs}.${nens}; fi
        ifile=gefs.t${cyc}z.${nens}.f${nfhrs}.alaska_3p0.grib2    
